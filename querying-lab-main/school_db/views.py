...
def problem_one(request):
  student_gpa = Student.objects.filter(gpa__gt = 3.0).order_by('-gpa')
...
def problem_three(request):
  instructor_key = Instructor.objects.get(id=2)
  print(f"Instructor Name: {instructor_key.first_name} {instructor_key.last_name} ")
  instructor_courses = Course.objects.filter(instructor_id=2)
  print(f"Instructor Course:")
...
def problem_four(request):
  ...
  print(f"Instructor Count: {instructor_count} ")
